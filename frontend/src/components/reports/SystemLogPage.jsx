...
const SystemLogPage = () => {
        ...
        const fetchLogs = async () => {
            ...
            try {
                ...
                const res = await axios.get(
                    `${import.meta.env.VITE_API_URL}/api/reports/system-logs`
                );
    ...
    const handlePrint = () => {
        downloadReportPdf({
            ...
            subtitle: "Full System Audit Trail (All User & System Actions)",
            filename: "system-activity-log-report.pdf",
            filters: getFilterText(),
            columns: [
                { label: "S No", value: (row, index) => index + 1 },
                { label: "User", value: (log, index) => getUser(log) },
                { label: "Action", value: (log, index) => log.action || '-' },
                { label: "Type", value: (log, index) => log.type || '-' },
                { label: "Time", value: (log, index) => log.createdAt ? new Date(log.createdAt).toLocaleString() : '-' },
            ],
            rows: filteredLogs ,
        });
    };
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div>
                            ...
                            <p className="page-subheading mb-0">
                                Full System Audit Trail (All User & System Actions)
                            </p>
